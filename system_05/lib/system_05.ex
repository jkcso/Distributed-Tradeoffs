...
defp create(num_peers, reliability) do
  # LOCAL RUN
  peers= Enum.map(0..(num_peers-1), fn(num) -> spawn(Peer, :start, [num,reliability]) end)
  IO.puts("Reliability : #{reliability}")
  for p <- peers, do: send p, { :bind, self(), peers }
  create_lpl(0, [], num_peers)
end
